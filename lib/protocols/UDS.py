...
class UdsClient():
    ...
    def access_timing_parameter(self, timing_parameter_type: TIMING_PARAMETER_TYPE, parameter_values: bytes = None):
        ...
        resp = self._uds_request(SERVICE_TYPE.ACCESS_TIMING_PARAMETER, subfunction=timing_parameter_type,
    ...
    def read_data_by_identifier(self, data_identifier_type):
        ...
        if resp_id != data_identifier_type:
            raise ValueError(
                'invalid response data identifier: {} expected: {}'.format(hex(resp_id), hex(data_identifier_type)))
        return resp.hex()


    def read_memory_by_address(self, memory_address: int, memory_size: int, memory_address_bytes: int = 4,
                               memory_size_bytes: int = 1):
    ...
    def read_data_by_periodic_identifier(self, transmission_mode_type: TRANSMISSION_MODE_TYPE,
    ...
    def dynamically_define_data_identifier(self, dynamic_definition_type: DYNAMIC_DEFINITION_TYPE,
        ...
        if dynamic_definition_type == DYNAMIC_DEFINITION_TYPE.DEFINE_BY_IDENTIFIER:
            for i in range(len(source_definitions)):
                data += struct.pack("!H", source_definitions[i])
                data += struct.pack("!B", start_point[i])
                data += struct.pack("!B", length[i])
        ...
        self._uds_request(SERVICE_TYPE.DYNAMICALLY_DEFINE_DATA_IDENTIFIER, subfunction=dynamic_definition_type,
                          data=data)
    ...
    def read_dtc_information(self, dtc_report_type: DTC_REPORT_TYPE,
                             dtc_status_mask_type: DTC_STATUS_MASK_TYPE = DTC_STATUS_MASK_TYPE.ALL,
                             dtc_severity_mask_type: DTC_SEVERITY_MASK_TYPE = DTC_SEVERITY_MASK_TYPE.ALL,
                             dtc_mask_record: int = 0xFFFFFF,
    ...
    def input_output_control_by_identifier(self, data_identifier_type, control_parameter_type: CONTROL_PARAMETER_TYPE,
                                           control_option_record: bytes = b'', control_enable_mask_record: bytes = b''):
        data = struct.pack('!H', data_identifier_type) + bytes(
            [control_parameter_type]) + control_option_record + control_enable_mask_record
    ...
    def routine_control(self, routine_control_type: ROUTINE_CONTROL_TYPE,
                        routine_identifier_type: ROUTINE_IDENTIFIER_TYPE, routine_option_record: bytes = b''):
    ...
    def request_download(self, memory_address: int, memory_size: int, memory_address_bytes: int = 4,
                         memory_size_bytes: int = 4, data_format: int = 0x00):
